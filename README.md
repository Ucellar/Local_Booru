...
[Download Local Booru v1.4](https://github.com/Ucellar/Local_Booru/releases/tag/v1.4)
...
The main change in v1.4 is that the parser is now modular and asynchronous.

Added and reworked:

- **Blueprint parser** — a visual pipeline graph: MD5 sites, reverse branches, side queues, relay blocks, and local preparation.
- **Async reverse side queue** — slow reverse methods should no longer stall the main MD5 pipeline.
- **Per-site checkpoints** — the app remembers which sites have already been checked for each file and, after restart, continues only unfinished work.
- **SQLite recovery/write gate** — after a crash or disk error, database writes are blocked until integrity checks are completed.
- **Source/Post URL → MD5 relay** — found links can be converted into MD5 values and sent back through the regular MD5 sites.
- **rule34 image-key relay** — a separate side branch for 40hex image keys and hotlink/post location.
- **ATF pixel_hash locator** — ATF lookup through `media_assets.json` and `pixel_hash`.
- **e621 browser companion fallback** — a fallback API path through the browser context.
- **FHD interface mode** — Blueprint and panels are now more comfortable at 1920×1080.
- **MD5 in parser status** — the task table shows the current MD5.

Fixed:

- reverse search no longer blocks the main MD5 parser;
- TinEye no longer starts when it is disabled in settings;
- Blueprint respects parser toggles;
- found files should no longer remain in the old NO_MATCH state;
- rule34 image-key handling was moved out of the main MD5 path into a side queue;
- after an abnormal shutdown, the database is checked before writes are allowed;
- Blueprint editor crashes and block layout issues were fixed.
